...
export async function appendAssets(root: HTMLElement, assetsList: string[]) {
  ...
  assetsList.forEach(url => {
    // //dyloader.com/index.css -> true
    // //dyloader.com/index.css?timeSamp=1575443657834 -> true
    // //dyloader.com/index.css?query=test.js -> false
...
export function getUrl(entry: string, relativePath: string): string {
  ...
  // https://dyloader.com/dynamic/index.html + ./js/index.js -> https://dyloader.com/dynamic/js/index.js
  if (startWith(relativePath, './')) {
    ...
    // https://dyloader.com/dynamic/index.html + /js/index.js -> https://dyloader.com/js/index.js
    return `${origin}${relativePath}`;
  } else {
    // https://dyloader.com + js/index.js -> https://dyloader.com/js/index.js
    return `${origin}/${relativePath}`;
  }
}

/**
 * If script/link processed by dyloader, add comment for it
 */
export function getComment(tag: string, from: string, type: AssetCommentEnum): string {
  return `<!--${tag} ${from} ${type} by dyloader-->`;
}
